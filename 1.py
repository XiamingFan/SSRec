...
import collections
import dgl
from HSINDataLoader import parse_args , DataLoaderHSIN
import torch
import logging

from SSRec_main import get_logger


class HERec:
    def __init__(self, data, metapaths, args):
    ...
    def load_embedding(self, metapaths, num):
        ...
        for metapath in metapaths:
            sourcefile = './data/' + self.dataset + '/embedding/' + ''.join(metapath) + '.embedding.walks.0'
...
if __name__ == "__main__":
    ...
    data = DataLoaderHSIN(logger1, args, dataset)
    metapaths = [['ubu', 'ubcibu', 'ubcabu'],['bub', 'bcib', 'bcab']]
    user_metapaths = ['ubcibu', 'ubcabu']
    item_metapaths = ['bcib', 'bcab']

    user_metapaths = ['ubu_0.8.embedding', 'ubcibu_0.8.embedding', 'ubcabu_0.8.embedding']

    item_metapaths = ['bub_0.8.embedding', 'bcib_0.8.embedding', 'bcab_0.8.embedding']


    HERec(data,metapaths, args)
